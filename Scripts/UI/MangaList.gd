...
onready var folder_path :String = Config.config.get_value("storage", "manga")

func _ready():
	check_all_folders(folder_path)
	...
	if dir.open(folder_path) == OK:
		...
		while file_path != "":
			if(file_path == "script.json"):
				var dict = State.load_title(folder_path+"/"+file_path, folder_path)
				add_button(dict["title"], dict, folder_path)
			file_path = dir.get_next()
		dir.list_dir_end()
