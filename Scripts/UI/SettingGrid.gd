...
func update_values():
	...
	$ffmpeg_container2/op_ffmpeg.select(Config.config.get_value("storage", "ffmpegMode", 1))
...
func _on_SettingSave_pressed():
	...
	Config.config.set_value("storage", "ffmpegMode", $ffmpeg_container2/op_ffmpeg.get_selected())
